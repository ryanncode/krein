...
fn bad_cross_branch_amplitude(a: u64, b: u64) -> Complex<f64> {
    let g = gcd(a, b);
    let w = count_factors(g) as f64;
    let mut imag = 0.0;
    if a != b {
...
fn good_cross_branch_amplitude(a: u64, b: u64) -> Complex<f64> {
    let g = gcd(a, b);
    let w = count_factors(g) as i64;
    let unit_g = address_to_krein_unit(g);
    let basis = KreinCoord(1, 1);
...
fn get_spacings(slice: &[u64], f: fn(u64, u64) -> Complex<f64>) -> Vec<f64> {
    let size = slice.len();
...
fn main() {
    ...
    while n < 4 {
        let size = slice.len();
        println!("Level {}: size {}", 14 - n, size);
        let bad_spacings = get_spacings(&slice, bad_cross_branch_amplitude);
        println!("  Bad spacings : {:?}", bad_spacings);
        let good_spacings = get_spacings(&slice, good_cross_branch_amplitude);
        println!("  Good spacings: {:?}", good_spacings);
